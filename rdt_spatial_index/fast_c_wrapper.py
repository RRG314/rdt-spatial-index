...
class RDTCIndex(RDTFastIndex):
    ...
    def query(self, queries, radius, timing=False):
        ...
        return _rdt_query_c(
            ...
            np.ascontiguousarray(self._leaf_x0, dtype=np.float64),
            np.ascontiguousarray(self._leaf_y0, dtype=np.float64),
            np.ascontiguousarray(self._leaf_x1, dtype=np.float64),
            np.ascontiguousarray(self._leaf_y1, dtype=np.float64),
            np.ascontiguousarray(self._leaf_start, dtype=np.int64),
            np.ascontiguousarray(self._leaf_end, dtype=np.int64),
            np.ascontiguousarray(self._order, dtype=np.int64),
            self._px,
            self._py,
