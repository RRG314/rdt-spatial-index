# Documentation Index

This index is the fastest way to navigate the repository as a new visitor.

## Start Here

- [README.md](README.md): project overview, quickstart, implementation story.
- [REVIEWER_GUIDE.md](REVIEWER_GUIDE.md): v1-v4 map, evidence map, and caveats.
- [IMPLEMENTATIONS.md](IMPLEMENTATIONS.md): reference vs fast vs compiled vs legacy.
- [LIMITATIONS.md](LIMITATIONS.md): public-facing constraints and caveats.

## Installation and Usage

- [README.md](README.md#install)
- [IMPLEMENTATIONS.md](IMPLEMENTATIONS.md#compiled-implementations)
- [packages/rdt-spatial-index/README.md](packages/rdt-spatial-index/README.md): Node.js/npm package usage.

## Testing

- [TESTING.md](TESTING.md)
- [tests/README.md](tests/README.md)

## Benchmarks and Results

- [BENCHMARKS.md](BENCHMARKS.md)
- [RESULTS_SUMMARY.md](RESULTS_SUMMARY.md)
- [results/README.md](results/README.md)
- [V2_RESULTS.md](V2_RESULTS.md): adaptive v2 results, ablations, and limitations.
- [V3_RESULTS.md](V3_RESULTS.md): workload-aware v3 results and negative findings.
- [V4_RESULTS.md](V4_RESULTS.md): analytic v4 configuration results and caveats.

## Reproducibility

- [REPRODUCIBILITY.md](REPRODUCIBILITY.md)
- [publication/README.md](publication/README.md)
- [publication/BENCHMARK_METHODS.md](publication/BENCHMARK_METHODS.md)
- [publication/REPRODUCIBILITY.md](publication/REPRODUCIBILITY.md)

## Project Governance

- [CONTRIBUTING.md](CONTRIBUTING.md)
- [RELEASE_NOTES.md](RELEASE_NOTES.md)
- [CHANGELOG.md](CHANGELOG.md)
- [LICENSE](LICENSE)
- [CITATION.cff](CITATION.cff)

## Historical and Experimental Material

- [experiments/README.md](experiments/README.md)
- `experiments/v4_*.py`: v4 ablation, regret, and sensitivity scripts.
- [legacy/README.md](legacy/README.md)
- [MIGRATION_NOTES.md](MIGRATION_NOTES.md)
